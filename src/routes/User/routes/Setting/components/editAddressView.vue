<template>
    <v-secondary-nav :title="'编辑地址'"></v-secondary-nav>
                    ...
                    <div class="area_con ml10">
                        ...
                        <span class="u_area_select">
                            <select id="area" v-model="address.county" style="width:100px;height:20px;font-size:12px;" v-validate:county="['required']">
                                <option value="430102" selected>芙蓉区</option>
...
export default {
    ...
    methods: {
        ...mapActions(['updateReceiptAdress', 'showToast', 'getReceiptAdress']),
        toggleDefaultAddr(){
    ...
    route: {
        data ({to}){
            ...
            this.getReceiptAdress().then( (json) => {
                let find = json.find((item)=>{
                    return item.id == to.params.id
                })
                if(!find) {
                  this.failCallback();
                }
                this.$data.address = JSON.parse(JSON.stringify(find));
